...
export const CallPanel = ({ characterName, characterImage, promptId, initialMessage, onClose, isMinimized, onMinimize }: CallPanelProps) => {
    ...
    const playVoice = async (text: string) => {
        ...
        try {
            const response = await fetch('http://127.00.1:8000/api/prompt/speak/', {
                method: 'POST',
    ...
    useEffect(() => {
        if (isCallAccepted) {
            // 초기 인사말 재생 (약간의 딜레이 후)
            setTimeout(() => playVoice(initialMessage), 500);
        }
    }, [isCallAccepted]);
                ...
                <div className="call-chatlog-overlay">
                    ...
                    <div className="call-chatlog-input-area">
                        <input
                            ...
                            placeholder={isMuted ? "음소거 상태입니다." : "메시지를 입력하세요..."}
                            value={input}
                            onChange={(e) => setInput(e.target.value)}
                            onKeyDown={handleKeyDown}
                            disabled={isLoading || isMuted} // 로딩중이거나 음소거 상태면 입력 불가
                        />
                        <button
